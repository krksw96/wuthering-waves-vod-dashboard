...
const maxPlaylistPages = Math.min(Math.max(Number.parseInt(String(args.maxPlaylistPages || "30"), 10), 1), 30);
...
const expandedRows = details.flatMap((item) => {
  ...
  const strongTitleEvidence = related.test(snippet.title || "") || wutheringTitleTerms.test(snippet.title || "");
  ...
  if (date < start || date > end || excludedChannelIds.has(snippet.channelId) || !strongTitleEvidence || !koreanEvidence || disclosedAiUse) return [];
